...
class PvtConv(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chanels=3, dim=128, num_classes=1000, embed_dims=[128, 256, 512],
                 ...
                 depths=[4, 6, 3], sr_ratios=[4, 2, 1], num_stages=4, n_fliter_list=[3, 48, 96, 192]):
        super().__init__()
        # stage1:添加第一层的conv,减少了一层卷积是为了和pvt第二层拼接上
        # region:block1
        self.block1 = nn.Sequential(
            nn.Conv2d(in_channels=n_fliter_list[0],out_channels=n_fliter_list[1],kernel_size=3, stride=2, padding=1),
            nn.Conv2d(in_channels=n_fliter_list[1],out_channels=n_fliter_list[2],kernel_size=3, stride=2, padding=1),
            nn.Conv2d(in_channels=n_fliter_list[2],out_channels=n_fliter_list[3],kernel_size=3, stride=2, padding=1),
            torch.nn.Conv2d(in_channels=n_fliter_list[-1], out_channels=dim, stride=1, kernel_size=1, padding=0),
    ...
    def forward_features(self, x):
        ...
        x = self.pos_drop4(x + self.pos_embed4[:, :(n+1)])
